...
public class ModLoot {
    ...
    public static void onLootTableLoad(LootTableLoadEvent evt) {
        ...
        }else if(evt.getName().toString().equals("convfound:geode/geode")){
            ConvenientFoundation.LOG.info("###Loading Geode Injects###");
            //TODO: Add injector
    ...
    public static void injectEntry(LootPool l_pool,ResourceLocation pool,String name,int weight){
        LootEntry entry = new LootEntryTable(pool, weight, 0, new LootCondition[0], name);
        l_pool.addEntry(entry);
    }
}
